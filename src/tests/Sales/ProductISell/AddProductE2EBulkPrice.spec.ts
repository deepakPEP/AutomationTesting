...
// covering 113 testcases in this single e2e
  test('Complete Bulk Price - Add Product Flow without variants', async ({page}, testInfo) => {
    ...
    await test.step('Step 2: Add Pricing and MOQ Info', async () => {
      ...
      const firstMinStr = (JSON.parse(product.unit_price)?.[0]?.minQty ?? '').toString();
      await addProductPreviewPage.assertMOQ(page, firstMinStr || '1',product.unit || 'Pieces');
      await addProductPreviewPage.assertPrice( product?.unit_price || '100',product.unit_type,'Bulk Pricing');
